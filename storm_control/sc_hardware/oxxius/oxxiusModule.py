#!/usr/bin/env python
"""
HAL module for Oxxius laser control.
Based off of storm_control\sc_hardware\coherent\coherentModule.py, HAL module for Coherent laser control.
...
class OxxiusLaserFunctionality(amplitudeModule.AmplitudeFunctionalityBuffered):
    """
...
class OxxiusModule(amplitudeModule.AmplitudeModule):
    """
...
class OxxiusLaserboxx(OxxiusModule):
    ...
    def __init__(self, module_params = None, **kwds):
        ...
        import storm_control.sc_hardware.oxxius.laserboxx as lboxx
        self.laser = lboxx.Laserboxx(port = serial_port)

        if self.laser.getStatus():
            [pmin, pmax] = self.laser.getPowerRange()
            self.laser_functionality = OxxiusLaserFunctionality(device_mutex = self.device_mutex,
